...
def validate_and_update(uid,category,params):
	...
	if category == 'Simple': 
		if len(params['start_time']) !=4 or not params['start_time'].isdigit():
			errors['start_time'] = "HHMM"
		elif int(params['start_time'][0:2]) > 24 or int (params['start_time'][2:4]) > 60:
			errors['start_time'] = "check HH<24 and MM<60"
		if len(params['end_time']) !=4 or not params['end_time'].isdigit():
			errors['end_time'] = "HHMM"
		elif int(params['end_time'][0:2]) > 24 or int (params['end_time'][2:4]) > 60:
			errors['end_time'] = "check HH<24 and MM<60"
		if convert_int(params['start_time']) > convert_int(params['end_time']):
		...
		for p in param_list:
			if len(params[p]) !=4 or not params[p].isdigit():
				errors[p] = "error"
...
""" Retrieve a schedule in database """
def get_schedule(userid):
	my_schedule = {} 
	my_schedule['start_times'] = []
	my_schedule['end_times'] = []

	all_times = db_fetch_one("select * from schedule where user=%s" % userid) # All sensors in database
...
def main():
	try:
		...
		current_user = []
	return render_template('index.html',userdetails=current_user)
...
@app.route('/send_comments',methods=['GET','POST'])
def send_comments():
	try:
		if 'id' in session:
			current_user = get_user(session.get('id'))
	...
	if request.form['email'] != "" and request.form['comments'] != "":
...
""" The user has provided an email address to reset his/her password, send an email to this user with a specific link """
@app.route('/activate_password',methods=['GET','POST'])
def activate_password():
	if request.form['email'] and request.form['email'] != '': # an email has been provided 
		...
		if i > 0: # insert request into database and send an email to the user
			...
			server.sendmail(request.form['email'],MY_EMAIL, message + 'You have requested to reset your password, to activate use this link:\n' +\
				URL + '/password/'+secret)
			return render_template('index.html',message="An email has been sent, check your inbox")
			...
			return render_template('index.html',message="This email is not recognised, please register.")	
	
	else: #error
		return render_template('index.html',message="An error has occured, email address is not valid")


""" The user is using the special link sent by email """
@app.route('/password/<string:query>', methods=['GET','POST'])
def enter_password(query):
	if request.method == 'GET':
		...
		if dt != '' : # this link exists check 	
			...
			if alert == "ok" and request.form['pwd1'] == request.form['pwd2']: 	# passwords matching and time ok
				if email != '': # update database
					db_insert("update users set password=md5('%s') where email='%s'" %(request.form['pwd1'],email))
					return render_template('index.html',message="You have succesfully reset your password")
				else:
					return render_template('savepassword.html',error="Email address not valid")	
			else:
				return render_template('savepassword.html',error="Passwords need to match and time limit is 10 mins")	
...
def complete():
	error=""
	query="insert into users(username,password,alert_hours,alert_minutes"
	if request.form['username'] != "" and request.form['pwd1'] != "": # Check given fields 
	...
	if error != "": # form incomplete, an errors has occured
		...
		query += ") values('%s',md5('%s'),%s,%s"\
		       %(new_username,request.form['pwd1'],request.form['hours'],request.form['mins'])
		if "phone" in query:
		...
		query += ")"
		db_insert(query)		
		last_id = db_fetch_one("select userid from users where username='%s' and password=md5('%s')" \
			                    %(new_username,request.form['pwd1']))
		my_user = User(new_username,last_id[0])
		USERS.update({last_id[0]:my_user}) 
		return render_template('index.html',message="Welcome "+ new_username+ ". You have succesfully registered!")
...
@oid.loginhandler
def login():
	# Data in my form could be saved for later (used for the feature 'remember me')
	form = LoginForm() 
	
	# Load users allowed to login
	USER_NAMES = dict((u.name, u) for u in USERS.itervalues()) 
	
	# If the user and the password are correct, change URL otherwise try again!
	try:
...
def add_sensor():
	...
	db_insert("insert into sensors (sensorname,phonenumber,userref,flag,lastnotify) values ('%s','%s',%s,'off','%s')" \
		%(remove_chars(request.form['sensorname']),convert_string(request.form['sensorcontact']),current_user['userid'],dt))
...
""" The user has made some changes in the settings """
@app.route('/update',methods=['GET','POST'])	
@login_required
def save():
	all_settings = {} 							       				# This dictionary will contain all settings
	current_user = get_user(session.get('id'))         				# Get user details
	sensors_list = get_sensors(current_user['userid']) 				# Get sensor details
	...
	%(convert_string(request.form['phone']),request.form['email'],request.form['alert_hours'],request.form['alert_mins'],current_user['userid']))

	# RELOAD SETTINGS AND SHOW ERRORS
	sensors_list = get_sensors(current_user['userid']) # sensors have been updated
	current_schedule = get_schedule(current_user['userid']) # schedule has been updated
	all_settings['start_times'] = current_schedule['start_times'] 
	all_settings['end_times'] = current_schedule['end_times'] 
 	all_settings['switches'] = time_difference(sensors_list,current_user['userid']) 
...
@app.route('/api/ping/<string:query>', methods=['GET','POST'])
def ping_id(query):
...
@app.route('/api/notify/<string:query>', methods=['GET','POST'])
def notify_id(query):
...
def contact():
	try:
		...
		current_user = []
	return render_template('contact.html',userdetails=current_user)
...
def about():
	try:
		...
		current_user = []
	return render_template('about.html',userdetails=current_user)
...
@login_required
def settings():
	# Look up for user and details (email,phone,alert settings)
	current_user = get_user(session.get('id')) 
	# Look up for sensors for this user
	sensors_list = get_sensors(current_user['userid']) 
	# Look up for schedule for this user
	current_schedule = get_schedule(current_user['userid']) 
	
	# Initialise all sensors, calculate last ping and get sensor timetable
	all_settings = {} 
	all_settings['switches'] = time_difference(sensors_list,current_user['userid'])     
	all_settings['start_times'] = current_schedule['start_times']
	all_settings['end_times'] = current_schedule['end_times']
	...
	return render_template('u.html',userdetails=current_user,mysettings=all_settings)	
